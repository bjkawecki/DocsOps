...
main() {
  ...
  local project update_run_container image
  ...
  docker run -d --name "$update_run_container" \
    ...
    -e "DOCSOPS_HEALTH_URL=${DOCSOPS_HEALTH_URL:-http://host.docker.internal/health}" \
    "$image" \
    bash /opt/docsops/scripts/update.sh "$version"
