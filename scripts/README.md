...
| [`docsops-demo`](docsops-demo)                           | VM-Lab / Demo: Landing + App (`docsops.local` / `demo.docsops.local`), cron reset     |
...
| [`release/build-bundle.sh`](release/build-bundle.sh)     | Build `docsops-vX.Y.Z.tar.gz` (CI / release; includes lab Landing dist)               |
...
DOCSOPS_BUNDLE_PATH=/path/docsops-v0.1.0.tar.gz DOCSOPS_SKIP_IMAGE_PULL=1 \
  sudo -E ./scripts/docsops-demo install --version v0.1.0
```

Siehe [docs/install.md](../docs/install.md) (Abschnitt VM-Lab).
