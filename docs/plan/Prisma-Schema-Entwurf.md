...
- **archivedAt:** Optional (DateTime?). Archivierte Dokumente erscheinen nicht in Catalog und normalen Listen; nur in der Archive-Ansicht (Tab auf Personal, Company, Department, Team). Lesbar und per PATCH entarchivierbar (archivedAt = null). Rechte: Archivieren/Entarchivieren wie Bearbeitung (canWrite).
