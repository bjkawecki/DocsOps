...
[x] **Document views (Recording + Explore):** View-Zähler für Popularität/Explore-Sortierung; Dedup **pro User + UTC-Kalendertag** (`DocumentView`); Aggregat `Document.viewCount`; Recording bei `GET /documents/:id` für published, nicht-trashed Docs mit Kontext; **Scope-Leads / Context-Write zählen nicht** (`canWriteContext`); Explore `orderBy viewCount desc, publishedAt desc`. Kein öffentlicher UI-Zähler in der Document-Leseansicht (Details-Sidebar); kein Retention-Job.

[x] **Document views – Lead-Auswertung (Most read):** Scope-Leads (Company/Department/Team Lead, Personal-Owner, Admin) sehen eine Auswertung **pro Scope** – nicht Plattform-Admin-only, nicht für normale Leser. **UI:** Peer zu Trash/Archive in der **Scope-Sidebar** (Context-Workspace-Chrome), Link „Most read“ → `/…/most-read`. **Darstellung v1:** Top-10 published Docs im Scope, sortiert nach `viewCount` (All time); Titel + Reader-days, Link zum Dokument; Empty „No reader activity yet.“ API `GET /me/most-read` nur mit `isScopeLead` (Personal: Owner). Kein Zeitraum-Filter in v1.
...
[x] **Dokument zwischen Kontexten verschieben (Phase 1 – Sofort-Move, gleicher Owner-Scope):** Explizite Move-Aktion `POST /documents/:id/move` `{ targetContextId }` für Dokumente mit bestehendem Kontext. Abgrenzung zu „Assign to context“ (nur kontextfreier Draft → erster Kontext via PATCH). Rechte: `canWriteContext` auf Quelle und Ziel (Scope-Lead / Personal-Owner / Admin); Same-Owner über `getContextOwnerId`. Tags bleiben gültig (gleicher Tag-Owner); Grants unverändert. PATCH A→B abgelehnt. UI: Document-Menü „Move to context“ wenn `canMove`. Notification `document-moved`. Audit leicht (Log + Inbox). Cross-Owner-Freigabe: siehe nächster Punkt.
[x] **Move-Freigabe (abgestuft, Phase 2):** **Owner-Wechsel** (anderes Team/Department/Company bzw. Personal↔Org): `DocumentMoveRequest` (pending/accepted/rejected/withdrawn); Antrag = `canWriteContext(source)`, Accept/Reject = `canWriteContext(target)`, Withdraw jederzeit durch Source-Write solange pending. Bei Accept: Tags strippen, Grants behalten. Max. ein pending Request pro Dokument. API: `POST …/move-requests`, accept/reject/withdraw, `GET /me/move-requests`. Phase-1 Cross-Owner weiter 409 mit Hinweis. UI: Sidebar **Approvals** (`/approvals`, Redirect `/reviews`; Content-Sidebar Reviews | Move requests); Move-Modal same-owner Sofort / cross-owner Request; Document-Menü Accept/Reject/Withdraw. Notifications `document-move-requested|accepted|rejected|withdrawn` (+ `document-moved` bei Accept).
...
[ ] **Reset einmal täglich:** Job/`docsops-demo reset` – DB + Demo-MinIO auf Seed; Banner „Demo resets daily“; Sessions invalidieren. _(Lokal: `pnpm --filter backend demo:reset` mit `DEMO_MODE=true`.)_
...
**Zielbild:** Drei getrennte Ebenen – **Context** (process / project / subcontext = wo), optionaler **Document type** (welche Art Doc), **Template** (Starter-Inhalt zu einem Type). Beim Anlegen: Type/Template wählen; Vorschau zeigt **when to use** (volle Definition, keine Mini-Beispiele), **example title**, **typical chapters**; Draft erhält Blocks und optional gesetzten Type (`null` erlaubt).

**Built-in** (Plattform, schlanker Kern) plus **Custom**: Scope Lead (Company/Department/Team) und Admin (org-/plattformweit). v1: ein Default-Template pro Type. Picker darf nach „often used in process/project“ filtern, ohne harte Pflicht. Ideensammlung / ausführliche Gliederungen: [Dokument-Templates.md](Dokument-Templates.md); SSoT der Built-ins: `builtinDocumentTemplatesCatalog.ts`.
...
| **ADR**                   | Project       | Architekturentscheidung | Eine technische Entscheidung und Trade-offs               |
| **Architecture overview** | Project       | Architekturübersicht    | Lebende Systemkarte (Kontext, Komponenten, Integrationen) |
...
**Nicht mehr Built-in** (bei Bedarf als **Custom** Type/Template): Baseline, Playbook, Checklist, Repository documentation, Meeting notes, Known issue. Bestehende Dokumente mit alten `builtin:…`-Keys behalten den Key in der DB; der Picker listet sie nicht mehr.

[x] **Konzept:** Optional `documentType` am Dokument; Type vs Template (Starter: example title, chapters, when-to-use); Built-in vs Custom (Scope + Admin/Company); Create-Flow + Verwaltungs-UI unter Catalog; Redirect `/document-templates` → `/templates` – festgehalten in diesem Abschnitt, [Dokument-Templates.md](Dokument-Templates.md) und [ADR 008](../platform/adr/008-document-types-templates.md).
...
[x] **Inhalt:** Built-in-Kern (8 Types) an Type+Template-Modell angebunden; DE↔EN-Mapping für Picker-Tooltips; when-to-use als Definitionsabsatz.
