...
export type ScopeRoleTier = 'admin' | 'company' | 'department' | 'teamLead' | 'teamMember' | 'none';
...
function hasTeamAssignment(user: LoadedUser): boolean {
  return user.teamMemberships.length > 0 || user.leadOfTeams.length > 0;
}
...
export async function assertCanAssignScopeRole(
  prisma: PrismaClient,
  args: { userId: string; kind: ScopeAssignmentKind; teamId?: string }
): Promise<void> {
  ...
  if (kind === 'companyLead') {
    if (hasTeamAssignment(user)) {
      throw new ScopeAssignmentConflictError(
        'User already has a team assignment. Remove team roles before assigning company lead.'
      );
    }
    return;
  }
  ...
  if (user.leadOfTeams.length > 0) {
    if (kind === 'teamMember') {
      throw new ScopeAssignmentConflictError(
        'User already has a team lead assignment. Remove the team lead role before assigning team membership.'
      );
    }
